...
import { ALL_BRANDS, ALL_CATEGORIES, DELETE_BRAND } from '@/utils/ApiRoutes';
...
export default function Index() {
    ...
    const [deleteData, setDeleteData] = useState<Brand | null>(null)
    ...
    const deleteConfirm = (brand: Brand) => {
        setShowModal(true)
        setDeleteData(brand);
    };
    ...
    const getConfirmationDelete = async (confirm: boolean) => {
        if (confirm) {
            try {
                const res = await axiosAuth.delete(DELETE_BRAND(deleteData?.id));
                        ...
                        <div className="w-full mb-3">
                            <div className="bg-white dark:bg-gray-800 text-gray-900 dark:text-white shadow-xl">
                                <div className="relative overflow-x-auto">
                                    <Form categories={categories} brand={brand} submissionCallback={() => {getAllBrands();setBrand(undefined)}} noUpdateCallback={() => {getAllBrands();setBrand(undefined)}} cancelCallback={() => setBrand(undefined)} />
                                </div>
                            </div>
                        </div>
                    </div>
                                        ...
                                        <CardContent>
                                            {/* {(!isLoading) ? (
                                                <DataTable
                                                    ...
                                                    data={brands}
                                                    items={brandData.meta?.total}
                                                    pageSize={take}
                                                    currentPage={brandPage}
                                                    lastPage={brandData.meta?.last_page}
                                                    onPageChange={onBrandPageChange}
                                                    isLoading={isLoading}
                                                />
                                            ) : (
                                            ...
                                            {/* {(!isLoading) ? ( */}
                                                <TableServerSide
                                                    columns={columns({ handleDelete, handleEdit })} // Pass callbacks
                                                    data={brands}
                                                    pageCount={brandData.meta?.last_page}
                                                    total={brandData.meta?.totalRecords}
