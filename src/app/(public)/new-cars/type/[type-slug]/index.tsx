...
import { ALL_BLOGS, GET_BLOG, ALL_MODELS_BY_SUBCATEGORY } from "@/utils/ApiRoutes";
...
export default function Index() {
	...
	const getBrand = async () => {
		try {
			...
			const res = await axiosAuth.get(ALL_MODELS_BY_SUBCATEGORY(typeSlug));
			if (res.data) {
								...
								brand?.models
									...
									?.map((model: Model) => (
										<div key={model.id} className="border rounded-xl shadow-xl flex flex-col md:col-span-1 bg-white">
											<Link title={brand.name} href={`/new-cars/${typeSlug}/${model.slug}`} >
